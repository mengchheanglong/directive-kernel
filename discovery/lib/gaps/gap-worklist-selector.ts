...
import type { DiscoveryGapWorklist, DiscoveryGapWorklistItem } from "./gap-worklist-generator.ts";
...
export type DiscoveryGapSelectionEligibleStatus =
  (typeof DIRECTIVE_DISCOVERY_GAP_SELECTION_ELIGIBLE_STATUSES)[number];

export type SelectedDiscoveryGap = Pick<
...
export type DiscoveryGapWorklistSelection = {
  directiveRoot: string;
  worklistRelativePath: string;
  worklistAbsolutePath: string;
  worklistUpdatedAt: string;
  selectionRule: string;
  eligibleStatuses: DiscoveryGapSelectionEligibleStatus[];
  totalOpenItems: number;
  eligibleOpenItems: number;
  selectedGap: SelectedDiscoveryGap | null;
};
...
export function isDiscoveryGapWorklistItemEligibleForSelection(
  ...
  gap_status: DiscoveryGapSelectionEligibleStatus;
} {
