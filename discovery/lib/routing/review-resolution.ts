...
import { writeGapRadarReport } from "../../../engine/routing/gap-radar.ts";
import {
  readDirectiveDiscoveryRoutingArtifact,
  type DiscoveryRoutingArtifact,
} from "./route-opener.ts";
...
function deriveReviewResolutionPath(routingRecordPath: string): string {
  ...
  return routingRecordPath.replace(/routing-record\.md$/, "review-resolution.md");
}
...
function validateDecisionAgainstRoute(
  decision: DiscoveryRoutingReviewDecision,
  routing: DiscoveryRoutingArtifact,
) {
...
export function writeDiscoveryRoutingReviewResolution(
  ...
  const gapRadar = writeGapRadarReport({
    directiveRoot,
