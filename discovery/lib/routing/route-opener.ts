...
import { readDiscoveryRoutingReviewResolution } from "./review-resolution.ts";
import {
  deriveEffectiveDiscoveryRouteBoundary,
  type DiscoveryRouteDestination,
} from "./effective-boundary.ts";
import { type DiscoveryIntakeQueueDocument } from "../intake/queue-writer.ts";
import { syncDiscoveryIntakeLifecycle } from "../intake/lifecycle-sync.ts";
import {
...
type EngineRunRoutingAssessment =
  NonNullable<EngineRunRecordLike["routingAssessment"]>;

export type DiscoveryRoutingArtifact = {
  title: string;
  date: string;
  candidateId: string;
  candidateName: string;
  ...
  matchedGapId: string | null;
  gapPressure: EngineGapPressureDetail | null;
  routingConfidence: string | null;
  routeConflict: boolean | null;
  needsHumanReview: boolean | null;
  digest: EngineRunRoutingAssessment["digest"] | null;
  missionSpecificityWarning: string | null;
  missionHealth: EngineRunRoutingAssessment["missionHealth"] | null;
  ...
  followUpQuestions: EngineRunRoutingAssessment["followUpQuestions"] | null;
  gapRadar: {
    summary: string;
    suggestions: Array<{
      radarId: string;
  ...
  sourceMemory: EngineRunRoutingAssessment["sourceMemory"] | null;
  sourceSimilarity: EngineRunRoutingAssessment["sourceSimilarity"] | null;
  narrativeContext: EngineRunRoutingAssessment["narrativeContext"] | null;
...
export type DiscoveryRouteOpenResult = {
  ok: true;
  created: boolean;
...
function isEngineRunRecordLike(value: unknown): value is EngineRunRecordLike {
  ...
  const matches = fs
    ...
    .map((recordPath) => {
      try {
        ...
        if (!isEngineRunRecordLike(parsed)) {
          return null;
        }
...
function inferRuntimeHostSelection(record: EngineRunRecordLike): {
...
function readRoutingArtifact(input: {
  ...
  return {
    ...
    gapPressure: engineRun?.record
      ? describeEngineGapPressure(engineRun.record)
      : null,
    routingConfidence:
