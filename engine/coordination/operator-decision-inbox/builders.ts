...
} from "../../../architecture/lib/materialization/due-check.ts";
import {
...
} from "../../../runtime/lib/control/promotion-assistance.ts";
import {
...
} from "../../mission/feedback-inbox.ts";
import {
...
const DISABLED_RUNTIME_AUTOMATION_POLICY: AutonomousRuntimePromotionAutomationPolicy = {
  ...
  return report.recommendations
    ...
    .map((recommendation) => {
      ...
      return attachPlanStateSummary({
        ...
        resolverCommandOrArtifact:
          recommendation.recommendedActionKind === "clarify_repo_native_host_target"
            ? `Use the operator inbox Runtime host-selection action, POST /api/runtime/selection-resolutions, or run "node --experimental-strip-types ./hosts/standalone-host/cli.ts runtime-host-selection-resolve --promotion-readiness-path \\"${recommendation.sourcePromotionReadinessPath}\\" --decision select_standalone --rationale \\"<operator rationale>\\"" to create ${hostSelectionResolutionPath}.`
...
export function buildRuntimeRegistryAcceptanceEntries(
  ...
  return listFiles({
    ...
    try {
      const dryRun = buildRuntimePromotionAutomationDryRunReport({
        directiveRoot,
        promotionRecordPath,
