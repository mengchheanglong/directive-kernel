...
import { buildDirectiveRunSourceTokenMap } from "../source-utils.ts";
import { deriveDirectiveMissionHealth } from "../mission/health.ts";
import { deriveEngineEarnedAutonomyAssessment } from "./earned-autonomy.ts";
import { deriveDirectiveFollowUpQuestionSet } from "./follow-up-questions.ts";
import {
  compileGapRadarSuggestions,
  deriveGapRadarAssessment,
} from "./gap-radar.ts";
import {
  deriveRoutingCorrectionAdjustments,
} from "./correction-ledger.ts";
import { deriveRoutingDigest } from "./digest.ts";
import {
...
import {
  ...
  deriveSourceMemoryAssessment,
} from "./source-memory.ts";
import { deriveSourceSimilarityAssessment } from "./source-similarity.ts";
import {
...
export function assessEngineRouting(input: {
  ...
  const sourceMemorySnapshot = createSourceMemorySnapshot({
    runs: existingRuns,
    precomputedSourceTokens,
  });
  const narrativeThreadBuild = buildSourceNarrativeThreads({
    runs: existingRuns,
    mission: input.mission,
  });
  const provisionalLaneId = deriveRecommendedLane(laneScores);
  const sourceMemoryPreAssessment = deriveSourceMemoryAssessment({
  ...
  const narrativeContextPreAssessment = deriveSourceNarrativeContext({
    source: input.source,
    sourceText,
    mission: input.mission,
    existingRuns,
  ...
  const gapRadarSuggestions = compileGapRadarSuggestions({
    events: policyEvents,
    openGaps: input.openGaps,
  });
  const gapRadar = deriveGapRadarAssessment({
    sourceText,
    recommendedLaneId,
    matchedGapId: matchedGap?.gapId ?? null,
    suggestions: gapRadarSuggestions,
  });
  const earnedAutonomy = deriveEngineEarnedAutonomyAssessment({
    source: input.source,
    recommendedLaneId,
  ...
  const narrativeContext = deriveSourceNarrativeContext({
    source: input.source,
    sourceText,
    mission: input.mission,
    existingRuns,
  ...
  const assessmentWithoutDigest: Omit<EngineRoutingAssessment, "digest"> = {
    recommendedLaneId,
    recommendedRecordShape,
