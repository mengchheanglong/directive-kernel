...
function priorityWeight(priority: EngineCapabilityGapPriority) {
  switch (priority) {
    case "high":
      return 3;
    case "medium":
...
function flattenSourceText(source: EngineSourceItem) {
  return [
    source.title,
    source.sourceRef,
...
function sortOpenGaps(openGaps: EngineCapabilityGap[]) {
  return [...openGaps]
...
function buildGapText(gap: EngineCapabilityGap) {
  return [
    gap.gapId,
    gap.description,
...
function findMatchedGap(input: {
  ...
  let bestGap: EngineCapabilityGap | null = null;
