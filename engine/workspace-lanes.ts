...
import type { EngineIntegrationProposal } from "./types.ts";

type WorkspaceLaneId = "discovery" | "architecture" | "runtime";

type WorkspaceLaneOverrides = Partial<
  Record<
    WorkspaceLaneId,
    Partial<
      Pick<
...
function createBaseLanes(): EngineLaneDefinition[] {
  return [
    {
      laneId: "discovery",
      label: "Discovery",
...
export function createDirectiveWorkspaceEngineLanes(input: {
  laneOverrides?: WorkspaceLaneOverrides;
} = {}): EngineLaneSet {
  const lanes = createBaseLanes().map((lane) => ({
    ...lane,
    ...(input.laneOverrides?.[lane.laneId as WorkspaceLaneId] ?? {}),
  }));
