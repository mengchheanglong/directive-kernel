...
export type StoredEngineRunRecord = {
  $schema?: string;
  ...
  executablePlanState?: EngineExecutablePlanState | null;
  reportPlan: {
    reportKind: string;
    summary: string;
    usefulnessRationale: string;
...
export type EngineGapPressureDetail = {
  openGapCount: number;
...
export type EngineGapPressureRecordLike = {
  candidate: {
    matchedGapId?: string | null;
  };
  openGaps?: Array<{
...
export type EngineRunArtifact = {
  recordPath: string;
  reportPath: string | null;
  reportExcerpt: string | null;
  reportContent?: string | null;
  gapPressure: EngineGapPressureDetail;
  record: StoredEngineRunRecord;
};
...
export type EngineRunDetail = {
  ...
  record: StoredEngineRunRecord | null;
};

export type EngineRunsOverview = {
  ok: boolean;
  error?: string;
  rootPath: string;
  engineRunsRoot: string;
  ...
  recentRuns: EngineRunArtifact[];
};

type ReadEngineRunsOverviewOptions = {
  directiveRoot?: string;
  maxRuns?: number;
};

type ReadEngineRunDetailOptions = {
  directiveRoot?: string;
  runId: string;
};
...
export function describeEngineGapPressure(
  record: EngineGapPressureRecordLike,
): EngineGapPressureDetail {
  const matchedGapId =
...
export function readEngineRunsOverview(
  options: ReadEngineRunsOverviewOptions = {},
): EngineRunsOverview {
...
export function readEngineRunDetail(
  options: ReadEngineRunDetailOptions,
): EngineRunDetail {
  const directiveRoot = normalizeAbsolutePath(options.directiveRoot || getDefaultDirectiveWorkspaceRoot());
  const engineRunsRoot = normalizeAbsolutePath(
    path.join(directiveRoot, "runtime", "standalone-host", "engine-runs"),
