...
function deriveAdaptationFollowThrough(input: {
  ...
  const actualCompletionRate = input.record.structuredAdaptationPlan?.completionRate ?? 0;
  if (actualCompletionRate >= 100) {
    input.rationale.push(
      "Adaptation follow-through is completed because the current structured adaptation plan is fully marked complete.",
    );
    return "completed" as const;
  }
  if (actualCompletionRate > 0) {
    input.rationale.push(
      `Adaptation follow-through is partial because the current structured adaptation plan is ${actualCompletionRate}% complete.`,
    );
...
function deriveImprovementGoalProgress(input: {
  ...
  const actualCompletionRate = input.record.structuredImprovementPlan?.completionRate ?? 0;
