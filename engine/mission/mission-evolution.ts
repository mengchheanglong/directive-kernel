...
export type MissionEvolutionRecord = {
  ...
  missionSnapshot: DirectiveEngineMissionContext;
  appliedDelta: DirectiveEngineMissionPreviewChange;
  trigger: {
...
export type MissionChangePreview = {
  previewId: string;
  generatedAt: string;
  currentMission: DirectiveEngineMissionContext;
  proposedMission: DirectiveEngineMissionContext;
  affectedRuns: Array<{
    runId: string;
    currentLane: string;
    currentConfidence: DirectiveEngineRoutingConfidence;
    projectedLane: string;
    projectedConfidence: DirectiveEngineRoutingConfidence;
...
function normalizeMissionContext(
  mission: DirectiveEngineMissionInput | DirectiveEngineMissionContext,
): DirectiveEngineMissionContext {
...
function normalizeMissionPreviewChange(
  input: DirectiveEngineMissionPreviewChange,
): DirectiveEngineMissionPreviewChange {
  return {
    objective:
...
function confidenceRank(value: DirectiveEngineRoutingConfidence) {
...
export function readCurrentMissionSnapshot(input?: {
  directiveRoot?: string;
  fallbackMission?: DirectiveEngineMissionInput | DirectiveEngineMissionContext | null;
}) {
...
function deriveCascadeEligibilityKind(run: DirectiveEngineRunRecord) {
...
export function previewMissionEvolution(input: {
  directiveRoot?: string;
  proposedMission: DirectiveEngineMissionInput | DirectiveEngineMissionContext;
  currentMission?: DirectiveEngineMissionInput | DirectiveEngineMissionContext | null;
  ...
  const affectedRuns = input.existingRuns
    .map((run) => {
      const projectedAssessment = assessDirectiveEngineRouting({
        source: run.source,
...
export function supersedeMissionEvolution(input: {
  directiveRoot?: string;
  newMissionSnapshot: DirectiveEngineMissionInput | DirectiveEngineMissionContext;
  operatorRationale: string;
