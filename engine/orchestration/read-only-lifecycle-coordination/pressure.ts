...
function buildPressure(
  ...
  entries: ReadOnlyLifecycleCoordinationEntry[],
): ReadOnlyLifecycleCoordinationPressure | null {
  if (entries.length === 0) {
    return null;
  }
...
export function selectTopPressure(entries: ReadOnlyLifecycleCoordinationEntry[]) {
  ...
  const pressures = [...grouped.entries()]
    ...
    .filter((pressure): pressure is ReadOnlyLifecycleCoordinationPressure => pressure !== null)
    .sort((left, right) => {
