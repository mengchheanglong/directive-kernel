...
const EXAMPLES_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)));

async function main() {
  const flags = parseArgs(process.argv.slice(2));
  const directiveRoot = path.resolve(
    flags["directive-root"] || "./local/bug-report-triage/directive-root",
  );
  const goalPath = path.resolve(
    flags["goal-json-path"] || path.join(EXAMPLES_ROOT, "goal-envelope.json"),
  );
  const sourcePath = path.resolve(
    flags["source-json-path"] || path.join(EXAMPLES_ROOT, "sample-source.json"),
  );

  const goal = readJson<FirstHostGoalEnvelopeInput>(goalPath);
  const source = readJson<FirstHostSourceInput>(sourcePath);

  const result = await runFirstHostIntegrationFlow({ directiveRoot, goal, source });
