/**
 * Tremor Callable Capability Executor
 *
 * Implements the Runtime callable contract for Tremor.
...
const CAPABILITY_ID = "hermes-tremor";
const TITLE = "Tremor Dashboard Components";
...
const COMPONENTS = [
  { name: "Card", category: "layout", description: "Dashboard card container for metrics and visualizations." },
  { name: "Metric", category: "data-display", description: "Large KPI/metric text primitive." },
  { name: "Area Chart", category: "charts", description: "Time-series area chart for trends and KPIs." },
  { name: "Bar Chart", category: "charts", description: "Categorical bar chart for comparisons." },
  { name: "Donut Chart", category: "charts", description: "Distribution visualization for segment shares." },
  { name: "Table", category: "data-display", description: "Tabular analytics presentation." },
];
...
function executeGetComponentDoc(input: Record<string, unknown>) {
  ...
  return {
    ...
    sourceUrl: "https://www.tremor.so/docs",
    repoUrl: "https://github.com/tremorlabs/tremor-npm",
  };
}
...
function executeGetInstallationGuide() {
  return {
    steps: ["pnpm add @tremor/react", "Ensure Tailwind CSS pipeline is configured", "Compose KPI cards and charts from Tremor primitives"],
    frameworks: ["Next.js", "Vite"],
    docsUrl: "https://www.tremor.so/docs",
    githubUrl: "https://github.com/tremorlabs/tremor-npm",
  };
}
...
export function createCallableCapability(): CallableCapability {
  ...
  if (!compliance.ok) {
    throw new Error(`Tremor callable contract violation: ${compliance.violations.join("; ")}`);
  }
  return capability;
}
