...
import type { RuntimeCallableExecutionRecord } from "../../core/callable-execution.ts";
import { readJson } from "../../../shared/lib/file-io.ts";
import { normalizeAbsolutePath } from "../../../shared/lib/path-normalization.ts";

export type RuntimeCallableExecutionStatusCount = {
  status: RuntimeCallableExecutionRecord["invocation"]["status"];
  count: number;
};
...
type CapabilityAccumulator = {
  ...
  latestExecutionAt: string | null;
  tools: Set<string>;
  statuses: Map<RuntimeCallableExecutionRecord["invocation"]["status"], number>;
};
...
function isExecutionRecord(value: unknown): value is RuntimeCallableExecutionRecord {
...
function toStatusCounts(
  counts: Map<RuntimeCallableExecutionRecord["invocation"]["status"], number>,
) {
...
export function buildRuntimeCallableExecutionEvidenceReport(options: {
  ...
  const statusCounts = new Map<RuntimeCallableExecutionRecord["invocation"]["status"], number>();
