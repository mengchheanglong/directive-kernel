...
export type RuntimeLoopControlReport = {
  ok: boolean;
  ...
  completionFrontier: {
    selectionState: CompletionSliceSelection["selectionState"];
    currentTarget: CompletionSliceSelection["currentTarget"];
    reason: string;
  ...
  loopSelection: {
    ...
    selectedDomain: RuntimeLoopSelectionDomain;
...
function pickLoopSelection(input: {
  completionSelection: CompletionSliceSelection;
  runtimeTopRecommendation: RuntimePromotionAssistanceRecommendation | null;
}): RuntimeLoopControlReport["loopSelection"] {
...
export function buildDirectiveRuntimeLoopControlReport(input?: {
  ...
  const completionSelection = selectNextCompletionSlice({
    directiveRoot,
  });
  ...
  const persistenceReport = buildBoundedPersistentCoordinationReport({
    directiveRoot,
    snapshotAt,
    dryRun: true,
  });
