...
import {
  ...
  type MirroredRuntimeCapabilityBoundaryOpenProjectionInput,
} from "../projections/capability-boundary-projections.ts";
import {
...
export type RuntimeProofArtifact = {
  title: string;
  candidateId: string;
...
export type RuntimeProofRuntimeCapabilityBoundaryOpenResult = {
  ok: true;
  created: boolean;
...
function buildRuntimeCapabilityBoundaryOpenProjectionInput(input: {
  ...
  approvedBy: string;
  snapshotAt: string;
}): MirroredRuntimeCapabilityBoundaryOpenProjectionInput {
  return {
