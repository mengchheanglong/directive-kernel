...
import {
  ...
  type MirroredRuntimeFollowUpOpenProjectionInput,
} from "../projections/follow-up-projections.ts";
...
export type RuntimeFollowUpArtifact = {
  title: string;
  candidateId: string;
...
export type RuntimeFollowUpOpenResult = {
  ok: true;
  created: boolean;
...
function buildRuntimeFollowUpOpenProjectionInput(input: {
  ...
  approvedBy: string;
  snapshotAt: string;
}): MirroredRuntimeFollowUpOpenProjectionInput {
  return {
...
export function readRuntimeFollowUpArtifact(input: {
  followUpPath: string;
  directiveRoot?: string;
}): RuntimeFollowUpArtifact {
  ...
  const artifact: RuntimeFollowUpArtifact = {
