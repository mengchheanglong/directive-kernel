...
import {
  ...
  type MirroredRuntimePromotionReadinessOpenProjectionInput,
} from "../projections/promotion-readiness-projections.ts";
import {
...
export type RuntimeRuntimeCapabilityBoundaryArtifact = {
  title: string;
  candidateId: string;
  ...
  content: string;
  proofArtifact: RuntimeProofArtifact;
  runtimeRecordArtifact: RuntimeRecordArtifact;
};

export type RuntimePromotionReadinessOpenResult = {
  ok: true;
  created: boolean;
...
function buildRuntimePromotionReadinessOpenProjectionInput(input: {
  ...
  approvedBy: string;
  snapshotAt: string;
}): MirroredRuntimePromotionReadinessOpenProjectionInput {
  return {
...
export function readDirectiveRuntimeRuntimeCapabilityBoundaryArtifact(input: {
  ...
  directiveRoot?: string;
}): RuntimeRuntimeCapabilityBoundaryArtifact {
...
export async function openDirectiveRuntimePromotionReadiness(input: {
  ...
  if (resolvedState.focus?.ok) {
    const currentMirror = readMirroredDiscoveryCaseRecord({
      directiveRoot,
      caseId: artifact.candidateId,
    });
