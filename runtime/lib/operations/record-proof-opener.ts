...
import {
  ...
  type MirroredRuntimeProofOpenProjectionInput,
} from "../projections/proof-open-projections.ts";
...
export type RuntimeRecordArtifact = {
  title: string;
  candidateId: string;
  ...
  content: string;
  followUpArtifact: RuntimeFollowUpArtifact;
};

export type RuntimeRecordProofOpenResult = {
  ok: true;
  created: boolean;
...
function buildRuntimeProofOpenProjectionInput(input: {
  ...
  approvedBy: string;
  snapshotAt: string;
}): MirroredRuntimeProofOpenProjectionInput {
  return {
