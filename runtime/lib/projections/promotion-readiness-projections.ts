...
} from "./projection-shared.ts";

export type MirroredRuntimePromotionReadinessOpenProjectionInput = {
  snapshotAt: string;
  approvedBy: string;
  boundaryDate: string;
  candidateId: string;
...
export type RuntimePromotionReadinessProjectionSet =
  | {
      ok: false;
      reason:
        | "missing_case_record"
...
export function materializeDirectiveRuntimePromotionReadinessProjectionSet(input: {
  ...
  caseId: string;
}): RuntimePromotionReadinessProjectionSet {
