...
describe("bug-report-triage example", () => {
  const tmpDir = path.join(os.tmpdir(), `dk-bug-triage-${randomUUID()}`);
  let result: Awaited<ReturnType<typeof runFirstHostIntegrationFlow>>;

  beforeAll(async () => {
    fs.mkdirSync(tmpDir, { recursive: true });
    const examplesRoot = path.resolve("hosts/integration-kit/examples/bug-report-triage");
