...
} from "../experiments/handoff-start.ts";
import { readCaseMirrorEvents } from "../../../engine/cases/case-event-log.ts";
import { readMirroredDiscoveryCaseRecord } from "../../../engine/cases/case-store.ts";

export type MirroredNoteArchitectureCloseoutProjectionInput = {
  snapshotAt: string;
  closedBy: string;
...
export type NoteArchitectureCloseoutProjectionSet =
  | {
      ok: false;
      reason:
        | "missing_case_record"
...
}): ArchitectureCloseoutWriteRequest {
...
export function materializeDirectiveNoteArchitectureCloseoutProjectionSet(input: {
  ...
  caseId: string;
}): NoteArchitectureCloseoutProjectionSet {
