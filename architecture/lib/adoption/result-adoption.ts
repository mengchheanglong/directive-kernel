...
} from "../experiments/closeout.ts";
import {
  assertDirectiveLifecycleTransitionAllowed,
  resolveDirectiveReviewFeedback,
} from "../experiments/lifecycle-review-feedback.ts";
import {
...
export type ArchitectureAdoptionDetail = {
...
export type ArchitectureResultAdoptionResult = {
  ok: true;
  created: boolean;
  snapshotAt: string;
  directiveRoot: string;
...
export type ArchitectureAdoptionFinalStatus =
  ArchitectureResultAdoptionResult["finalStatus"];
...
function inferValueShape(resultArtifact: ArchitectureBoundedResultArtifact): ArchitectureValueShape {
...
function resolveDirectiveArchitectureAdoptionReviewScore(input: {
  finalStatus: ArchitectureAdoptionFinalStatus;
...
): ArchitectureResultAdoptionResult {
...
export function readArchitectureAdoptionDetail(input: {
  adoptionPath: string;
  directiveRoot?: string;
}): ArchitectureAdoptionDetail {
  const directiveRoot = normalizePath(input.directiveRoot || getDefaultDirectiveWorkspaceRoot());
