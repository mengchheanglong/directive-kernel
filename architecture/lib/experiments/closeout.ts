...
} from "./handoff-start.ts";
import {
  upsertDirectiveArchitectureAdoptionDecisionArtifact,
} from "../adoption/decision-store.ts";
import {
...
import { appendCaseMirrorEvents, readCaseMirrorEvents } from "../../../engine/cases/case-event-log.ts";
import {
  syncDiscoveryIntakeLifecycle,
} from "../../../discovery/lib/intake/lifecycle-sync.ts";
...
import type {
  ArchitectureSelfImprovementArtifact,
...
export type ArchitectureBoundedResultArtifact = {
  title: string;
  candidateId: string;
  candidateName: string;
  experimentDate: string;
...
export type ContinueArchitectureBoundedResultInput = {
  resultPath: string;
  directiveRoot?: string;
  continuedBy?: string | null;
};

export type ArchitectureContinuationStartResult = {
  ok: true;
  created: boolean;
  snapshotAt: string;
  directiveRoot: string;
...
export type CloseArchitectureBoundedStartInput = {
  startPath: string;
  directiveRoot?: string;
...
export type CloseArchitectureNoteHandoffInput = {
  handoffPath: string;
  directiveRoot?: string;
  closedBy?: string | null;
...
export type ArchitectureBoundedCloseoutResult = {
  ok: true;
  created: boolean;
  snapshotAt: string;
  directiveRoot: string;
...
export type ArchitectureBoundedCloseoutAssist = {
...
export type ArchitectureResultEvidenceSlot = {
...
type ArchitectureCloseoutAssistEngineRun = {
  analysis?: {
    missionFitSummary?: string;
    primaryAdoptionQuestion?: string;
  };
...
function collectWorkspaceFilePathMentions(text: string) {
  const matches = text.match(/[A-Za-z0-9._/-]+\.(?:ts|tsx|js|jsx|json|md)/g) || [];
  return Array.from(new Set(matches));
}

function resolveExistingWorkspaceRelativePath(input: {
  directiveRoot: string;
...
function resolveArchitecturePrimaryDirectEvidencePath(input: {
  directiveRoot: string;
  resultSummary: string;
  rationale: string;
}) {
  const candidates = collectWorkspaceFilePathMentions(
    `${input.resultSummary} ${input.rationale}`,
  )
    .map((candidatePath) =>
      resolveExistingWorkspaceRelativePath({
...
function resolveRecordedArchitecturePrimaryEvidencePath(input: {
  directiveRoot: string;
  primaryEvidencePath?: string | null;
}) {
...
function resolveRecordedArchitectureProducedArtifacts(input: {
  directiveRoot: string;
  transformedArtifactsProduced?: string[] | null;
}) {
...
function readArchitectureCloseoutAssistEngineRun(input: {
  artifact: ArchitectureBoundedStartArtifact;
}) {
  ...
  try {
    ...
    ) as ArchitectureCloseoutAssistEngineRun;
    return parsed;
  } catch {
    return null;
  }
...
function buildArchitectureSuggestedResultSummary(input: {
  artifact: ArchitectureBoundedStartArtifact;
  stages: string[];
  directiveOwnedForm: string;
  intendedDelta: string;
...
export function readArchitectureBoundedCloseoutAssist(input: {
  startPath: string;
  directiveRoot?: string;
}): ArchitectureBoundedCloseoutAssist {
  const artifact = readArchitectureBoundedStartArtifact({
    directiveRoot: input.directiveRoot,
    startPath: input.startPath,
  });
  const engineRun = readArchitectureCloseoutAssistEngineRun({ artifact });
  ...
  return {
    ...
    decisionGuidance,
    readinessGuidance,
    suggestedResultSummary: buildArchitectureSuggestedResultSummary({
      artifact,
...
export function readArchitectureResultEvidenceForResult(input: {
  resultPath: string;
  directiveRoot?: string;
}): ArchitectureResultEvidenceSlot {
  const artifact = readArchitectureBoundedResultArtifact({
    directiveRoot: input.directiveRoot,
    resultPath: input.resultPath,
  });
  const primaryDirectEvidencePath =
    artifact.primaryEvidencePath
    ?? resolveArchitecturePrimaryDirectEvidencePath({
...
export function readArchitectureResultEvidenceForStart(input: {
  startPath: string;
  directiveRoot?: string;
}): ArchitectureResultEvidenceSlot {
  const artifact = readArchitectureBoundedStartArtifact({
  ...
  return readArchitectureResultEvidenceForResult({
    directiveRoot: artifact.directiveRoot,
...
function renderBoundedResultMarkdown(input: {
  startArtifact: ArchitectureBoundedStartArtifact;
  snapshotAt: string;
  closedBy: string;
  resultSummary: string;
  primaryEvidencePath: string | null;
...
function renderNoteHandoffBoundedResultMarkdown(input: {
  handoffArtifact: ArchitectureHandoffArtifact;
  snapshotAt: string;
  closedBy: string;
  resultSummary: string;
  primaryEvidencePath: string | null;
...
function renderContinuationBoundedStartMarkdown(input: {
  resultArtifact: ArchitectureBoundedResultArtifact;
  snapshotAt: string;
  continuedBy: string;
}) {
...
export function readArchitectureBoundedStartArtifact(input: {
  startPath: string;
  directiveRoot?: string;
}): ArchitectureBoundedStartArtifact {
...
export function readArchitectureBoundedResultArtifact(input: {
  resultPath: string;
  directiveRoot?: string;
}): ArchitectureBoundedResultArtifact {
  const directiveRoot = normalizePath(input.directiveRoot || getDefaultDirectiveWorkspaceRoot());
...
export function continueArchitectureFromBoundedResult(
  input: ContinueArchitectureBoundedResultInput,
): ArchitectureContinuationStartResult {
  const resultArtifact = readArchitectureBoundedResultArtifact({
    directiveRoot: input.directiveRoot,
    resultPath: input.resultPath,
  });
...
export async function closeArchitectureNoteHandoff(
  ...
  if (created) {
    const primaryEvidencePath = resolveRecordedArchitecturePrimaryEvidencePath({
      directiveRoot: handoffArtifact.directiveRoot,
      primaryEvidencePath: input.primaryEvidencePath,
    });
    const transformedArtifactsProduced = resolveRecordedArchitectureProducedArtifacts({
      directiveRoot: handoffArtifact.directiveRoot,
    ...
    const projectionInput: MirroredNoteArchitectureCloseoutProjectionInput = {
      snapshotAt,
      closedBy,
      resultSummary,
      primaryEvidencePath,
  ...
  if (resolvedState.focus?.ok) {
    ...
    const eventLog = readCaseMirrorEvents({
      directiveRoot: handoffArtifact.directiveRoot,
      caseId: handoffArtifact.candidateId,
    });
    const nextSequence = eventLog.events.reduce(
      (highest: number, event: { sequence: number }) => Math.max(highest, event.sequence),
      0,
    ) + 1;
...
export function closeArchitectureBoundedStart(
  input: CloseArchitectureBoundedStartInput,
  ...
  const closeoutRequest: ArchitectureCloseoutWriteRequest = {
