...
export type ArchitectureHandoffStartResult = {
  ok: true;
  created: boolean;
  snapshotAt: string;
...
export type ArchitectureHandoffArtifact = {
  title: string;
  date: string;
  status: string;
  candidateId: string;
...
function readArchitectureHandoffArtifactByRelativePath(input: {
  directiveRoot: string;
  handoffRelativePath: string;
}): ArchitectureHandoffArtifact {
...
export function readArchitectureHandoffArtifact(input: {
  handoffPath: string;
  directiveRoot?: string;
}): ArchitectureHandoffArtifact {
  ...
  return readArchitectureHandoffArtifactByRelativePath({
    directiveRoot,
    handoffRelativePath,
  });
}

export function listArchitectureHandoffArtifacts(input: {
  directiveRoot?: string;
  maxEntries?: number;
} = {}) {
  const directiveRoot = normalizeDirectiveWorkspaceRoot(input.directiveRoot);
  ...
  return fs
    ...
    .map((entry) =>
      readArchitectureHandoffArtifactByRelativePath({
        directiveRoot,
...
export function startDirectiveArchitectureFromHandoff(input: {
...
}): ArchitectureHandoffStartResult {
  const directiveRoot = normalizeDirectiveWorkspaceRoot(input.directiveRoot);
  const handoffRelativePath = resolveDirectiveWorkspaceRelativePath(
    directiveRoot,
    input.handoffPath,
  ...
  const artifact = readArchitectureHandoffArtifactByRelativePath({
    directiveRoot,
    handoffRelativePath,
  });
