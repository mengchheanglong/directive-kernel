...
} from "../materialization/post-consumption-evaluation.ts";
...
export type ArchitectureReopenedStartResult = {
  ok: true;
  created: boolean;
  snapshotAt: string;
  directiveRoot: string;
...
function renderReopenedBoundedStartMarkdown(input: {
  ...
  reopenedBy: string;
  evaluationRelativePath: string;
  evaluationDetail: ReturnType<typeof readArchitecturePostConsumptionEvaluationDetail>;
  sourceResult: ArchitectureBoundedResultArtifact;
  reopenedStartRelativePath: string;
}) {
...
export function reopenDirectiveArchitectureFromEvaluation(
...
): ArchitectureReopenedStartResult {
  ...
  const evaluationDetail = readArchitecturePostConsumptionEvaluationDetail({
    directiveRoot,
