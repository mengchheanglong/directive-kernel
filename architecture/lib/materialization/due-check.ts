...
import type {
  ArchitectureCycleDecisionSummary,
} from "../adoption/cycle-decision-summary.ts";
import {
...
} from "./implementation-target.ts";
import {
  readDirectiveArchitectureImplementationResultPathForTarget,
} from "./implementation-result.ts";
import {
  readArchitectureAdoptionDetail,
} from "../adoption/result-adoption.ts";
import { ARCHITECTURE_DEEP_TAIL_STAGE } from "../control/materialization-tail-stage-map.ts";
import {
...
export type ArchitectureMaterializationDueKind =
  | "create_implementation_target"
  | "record_implementation_result";

export type ArchitectureMaterializationDueItem = {
  candidateId: string;
  candidateName: string;
  usefulnessLevel: string;
  dueKind: ArchitectureMaterializationDueKind;
...
export type ArchitectureMaterializationDueCheck = {
  ok: true;
  snapshotAt: string;
  ...
  warnings: string[];
  dueItems: ArchitectureMaterializationDueItem[];
};
...
export function readDirectiveArchitectureMaterializationDueCheck(input: {
  ...
  for (const adoptionRelativePath of listMarkdownRelativePaths(directiveRoot, "architecture/02-adopted")) {
    ...
    let adoptionDetail: ReturnType<typeof readArchitectureAdoptionDetail>;
    try {
      adoptionDetail = readArchitectureAdoptionDetail({
        directiveRoot,
  ...
  for (const targetRelativePath of listImplementationTargetRelativePaths(directiveRoot)) {
    let targetDetail: ReturnType<typeof readArchitectureImplementationTargetDetail>;
    try {
      targetDetail = readArchitectureImplementationTargetDetail({
        directiveRoot,
  ...
  let dueAdoptionDecisionSummary: ArchitectureMaterializationDueCheck["dueAdoptionDecisionSummary"] = null;
